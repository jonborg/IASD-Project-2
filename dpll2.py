...
def get_sat_sentence():
    
    f = open("dimacs_sentence.txt", "r")
    lines = f.readlines()
    ...
    for line in lines:
        ...
        if clause[0] == 'p':
            n_vars = int(clause[2])
            n_clauses = int(clause[3][:-1])
            continue
        clause = list(map(int, clause[:-1]))
        sentence.append(clause)
    
    return [n_vars, n_clauses, sentence]
...
def main ():
    ...
    [n_vars, n_clauses, sentence] = get_sat_sentence()
#    print(n_vars)
#    print(n_clauses)
#    print(sentence)
    
    setup = [copy.deepcopy(sentence), copy.deepcopy(sentence), [0] * n_vars, [], [], 0, []]
